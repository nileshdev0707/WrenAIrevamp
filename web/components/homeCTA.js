            ...
            <div key={index} className="grid sm:grid-cols-2 gap-8">
              <div className="flex flex-col justify-center">
                <div>
                  ...
                  <h3 className={`${item?.topTitle? 'mt-4' : ''} md:text-3xl text-2xl lg:text-4xl md:font-medium font-normal  max-w-md group-hover:opacity-95 transition-opacity duration-300`}>
                    {item?.title}
                  </h3>

                  <p className="text-sm opacity-90 mt-4 group-hover:opacity-100 transition-opacity duration-300">
                    {item?.subTitle}
                  </p>
                  <div className="flex justify-center sm:hidden ">
                    {url ? (
                      ...
                      <div className="text-gray-600 group-hover:opacity-80 transition-opacity duration-200">
                        {item?.name}
                      </div>
                    )}
                  </div>
                    ...
                    {item?.btnBottomContentBlock?.map((btn) => (
                      <div key={btn.id}>
                        <Button
                          ...
                          label={btn?.label}
                          target={
                            btn.url?.startsWith("http") ? "_blank" : "_self"
                          }
                          variant="elevated"
                        >
                          {btn?.label}
