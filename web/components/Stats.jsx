export default function Stats({ data }) {
  ...
  return (
    <section className="lg:py-6 md:py-5 sm:py-3 py-1">
      <div className="max-w-7xl mx-auto lg:px-6 sm:px-4 px-4">
        ...
        <div className="grid md:grid-cols-3 md:gap-6 sm:gap-4 gap-2 text-center 2xl:pt-20 xl:pt-15 lg:pt-14 md:pt-12 sm:pt-10 pt-8">
         ...
         <div className="grid md:grid-cols-3 sm:grid-cols-2 md:gap-6 sm:gap-5 gap-4 md:mt-9 sm:mt-6 mt-4"> 
           {stats?.teamsItem?.map((s, i) => (
             <div key={i} className="lg:px-8 lg:py-10 md:px-6 md:py-8 sm:px-5 sm:py-6 px-4 py-5 bg-white rounded-xl shadow-[0px_56px_232px_-56px_rgba(0,0,0,0.5)] flex flex-col justify-between">
                 <div className="font-medium text-[#1E1E1E] md:mb-8 mb-6 xl:text-xl lg:text-lg md:text-base text-base">{s.title}</div>
                  <div>
                    <div className="text-[#0B8EE5] lg:text-base md:text-sm text-sm font-bold mb-1">{s.name}</div>
                    <div className="text-[#757575] lg:text-base md:text-sm text-sm">{s.role}</div>
                  </div>
             </div>
           ))}
         </div>
