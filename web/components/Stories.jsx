...
export default function Stories({ data }) {
  ...
  useEffect(() => {
    ...
    return () => {
      try {
        if (window.jQuery?.fn?.slick && window.jQuery('.center').hasClass('slick-initialized')) {
    ...
    <section className="lg:py-20 md:py-15 py-10">
      <div className="mx-auto lg:px-6 md:px-4 px-2">
        <div className="text-center">
          ...
          <p className="text-[#757575] sm:text-base text-sm max-w-[550px] mx-auto">{caseStudies?.description}</p>
        </div>
                   ...
                   <p className='text-[#1E1E1E] xl:hidden text-center sm:pb-3 pb-2 md:pb-5  sm:pt-2 font-bold text-lg md:text-xl'>{story.subTitle}</p>
                     <div className='relative rounded-xl overflow-hidden group'>
                       {story.image[0]?.url ? (
                         <>
                           <img 
                             src={`${story.image[0]?.url?.startsWith('http') ? '' : base}${story.image[0]?.url}`} 
                             alt={story.title} 
                           ...
                           <div className='bg-black/70 absolute inset-0 w-full h-[250px] md:h-[350px] 2xl:h-[434px] object-cover opacity-0 transition-opacity duration-300 group-hover:opacity-100'>
                                <div className='flex items-center justify-center h-full'>
                                    {story.image[1]?.url && (
                                    <img 
                                      src={`${story.image[1]?.url?.startsWith('http') ? '' : base}${story.image[1]?.url}`} 
                                      alt={story.title} 
                          ...
                          <div className="text-gray-600 flex items-center justify-center h-[250px] md:h-[350px] lg:h-[434px]">{story.title}</div>
                        )}
                     </div>
                    </div>
                    <div className='items-center 2xl:col-span-2 xl:col-span-3 flex flex-col justify-center text-center md:text-left'>
                     <p className='py-2 hidden xl:block 2xl:text-7xl xl:text-6xl md:text-5xl lg:text-4xl text-xl bg-gradient-to-r from-[#0B8EE5] to-[#0022CB] bg-clip-text text-transparent font-bold'>{story.title}</p> 
                     <p className='text-[#1E1E1E] hidden xl:block pb-3 md:pb-5 pt-2 font-bold text-lg md:text-xl'>{story.subTitle}</p>
                     <div className='pb-4 md:pb-5 font-medium text-[#757575] max-w-md mx-auto md:mx-0 text-description text-base xl:text-xl lg:text-lg md:text-lg'>{story.description}</div>
                     <div>
                        <a href={story.link} className='text-sm text-gray-600 flex gap-2 justify-center md:justify-start items-center'>
                          <p className='text-sm md:text-base font-bold text-[#1E1E1E]'>
                             {story.linkTitle} 
                          </p>
                          <div className='min-w-6 w-6 h-6 md:min-w-7 md:w-7 md:h-7'>
                               <img src="/svg/gradientarrow.svg" alt="arrow" className="w-6 h-6 md:w-7 md:h-7 arrow-inactive" />
                               <img src="/svg/arrow.svg" alt="arrow" className="w-6 h-6 md:w-7 md:h-7 arrow-active hidden" />
                          </div>
                        </a>
                      </div>
                    </div>
